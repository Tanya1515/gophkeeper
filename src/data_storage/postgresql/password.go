...
func (pg *PostgreSQLConnection) UpdatePassword(ctx context.Context, password, app, md string, updatedAt time.Time, initVector []byte) error {
	...
	_, err = tx.ExecContext(ctx, "UPDATE Credentials SET "+
		...
		"updatedAt = $6 "+
		"WHERE Credentials.updatedAt <= $6 AND application=$3 AND userID=$1", ctx.Value(ut.IDKey), password, app, md, initVector, updatedAt)
