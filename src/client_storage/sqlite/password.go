...
func (cache *SQLite) GetPassword(application, userName string) (password string, metadata string, err error) {
	...
	row := db.QueryRowContext(ctxCache, "SELECT password, metadata, lastUpdated, accessCount, uploadTime WHERE application=$1 AND userName=$2", application, userName)
...
func (cache *SQLite) UploadPassword(application, password, metadata, uploadTime, userName string) (err error) {
	...
	_, err = db.ExecContext(ctxCache, "INSERT INTO Passwords (application, password, metadata, lastUpdated, uploadTime, userName) VALUES ($1,$2,$3,$4,$5,$6) "+
		"ON CONFLICT (application, userName) DO "+
		"UPDATE SET "+
		...
		"Passwords.accessCount = Passwords.accessCount + 1 WHERE Passwords.application = excluded.application AND Passwords.userName = excluded.userName", application, password, metadata, uploadTime, uploadTime, userName)
